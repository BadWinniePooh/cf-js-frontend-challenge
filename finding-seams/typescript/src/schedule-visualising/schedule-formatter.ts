...
export class ScheduleFormatter {
  format(scheduleObject: ScheduleObject | null | undefined, options: ScheduleFormatOptions = {}): string {
    const { timezoneOffset = 0, conferenceDay, timezone = 'UTC' } = options
  ...
  #shiftTime(timeStr: string, offsetMinutes: number): string {
